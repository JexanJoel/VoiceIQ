...
export default function CallsPage() {
  ...
  // Filter calls by date if date param present
  const filteredCalls = dateFilter
    ? calls.filter(c => new Date(c.created_at).toISOString().split('T')[0] === dateFilter)
    : calls
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <p style={{ color: '#6B7280', fontSize: '14px', marginTop: '2px' }}>
            {dateFilter ? `Showing calls from ${dateFilter}` : `${calls.length} calls processed`}
          </p>
        </div>
      ...
      {/* Date filter banner */}
      {dateFilter && (
        <div style={{ background: '#EEF2FF', border: '1px solid #C7D2FE', borderRadius: '10px', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ fontSize: '13px' }}>📅</span>
          <span style={{ fontSize: '13px', color: '#3730A3', fontWeight: '600' }}>
            Filtered by date: {dateFilter} · {filteredCalls.length} call{filteredCalls.length !== 1 ? 's' : ''} found
          </span>
        </div>
      )}
      ...
      {showUploadModal && pendingFile && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: '20px' }}>
          <div style={{ background: '#fff', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '420px', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
            ...
            <p style={{ fontSize: '13px', color: '#9CA3AF', marginBottom: '20px' }}>📎 {pendingFile.name}</p>
            <div style={{ marginBottom: '20px' }}>
              ...
              {agents.length === 0 ? (
                <div style={{ padding: '10px 13px', borderRadius: '10px', border: '1px solid #E5E7EB', fontSize: '13px', color: '#9CA3AF', background: '#F9FAFB' }}>
                  No agents yet — <span style={{ color: '#E11D48', cursor: 'pointer', fontWeight: '600' }} onClick={() => { setShowUploadModal(false); navigate('/dashboard/agents') }}>create one first</span>
                </div>
              ) : (
                <select value={selectedAgent} onChange={e => setSelectedAgent(e.target.value)} style={sel}>
      ...
      {uploadError && (
        <div style={{ background: '#FEE2E2', color: '#991B1B', padding: '10px 14px', borderRadius: '8px', fontSize: '13px' }}>{uploadError}</div>
      )}

      {loading ? <Loader center /> : filteredCalls.length === 0 ? (
        <EmptyState
          icon={dateFilter ? '📅' : '📞'}
          title={dateFilter ? 'No calls on this date' : 'No calls yet'}
          description={dateFilter ? 'Try a different date or clear the filter' : 'Upload your first call recording to get started'}
        />
      ) : (
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', minWidth: 0 }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontWeight: '700', fontSize: '14px', color: '#111827', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{call.file_name}</div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                  <Badge variant={getComplianceBadge(call.sop_compliance_percentage) as any}>{call.sop_compliance_percentage}% SOP</Badge>
                  <Badge variant={call.sentiment === 'positive' ? 'success' : call.sentiment === 'negative' ? 'danger' : 'warning'}>{call.sentiment}</Badge>
                  <button
                    onClick={(e) => handleDelete(call.id, e)}
                    disabled={deletingId === call.id}
                    style={{ width: '32px', height: '32px', borderRadius: '8px', background: '#FEF2F2', border: '1px solid #FECACA', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: deletingId === call.id ? 'not-allowed' : 'pointer', fontSize: '14px', flexShrink: 0, opacity: deletingId === call.id ? 0.5 : 1, transition: 'all 0.15s ease' }}
...
const sel: React.CSSProperties = { width: '100%', padding: '10px 13px', borderRadius: '10px', border: '1px solid #E5E7EB', fontSize: '13px', outline: 'none', background: '#fff', color: '#111827', fontFamily: "'Plus Jakarta Sans', sans-serif" }