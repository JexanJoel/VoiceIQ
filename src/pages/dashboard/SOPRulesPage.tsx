...
import { getSopRules, createSopRule, deleteSopRule, toggleSopRule, updateSopRule, getViolationsByRule } from '../../services/api'
...
type Rule = { id: string; rule_text: string; category: string; is_active: boolean; created_at: string }
type RuleStat = { rule_id: string; violation_count: number }
type EditState = { id: string; rule_text: string; category: string } | null
...
export default function SOPRulesPage() {
  ...
  const fetchRules = async () => {
    try {
      const [rulesRes, statsRes] = await Promise.all([
        getSopRules(),
        getViolationsByRule().catch(() => ({ data: { data: [] } }))
      ])
      setRules(rulesRes.data.data)
      setRuleStats(statsRes.data.data || [])
    } catch {
  ...
  const getViolationCount = (ruleId: string): number => {
    const stat = ruleStats.find(s => s.rule_id === ruleId)
    return stat?.violation_count || 0
  }
  ...
  const handleUseDefaults = async () => {
    ...
    setError('')
    try {
      for (const r of DEFAULT_RULES) await createSopRule(r)
      await fetchRules()
    } catch {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '800px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
        ...
        {rules.length < MAX_RULES && (
          <Button size="sm" onClick={() => { setAdding(true); setError('') }}>+ Add Rule</Button>
        )}
      </div>

      {error && (
        <div style={{ background: '#FEE2E2', color: '#991B1B', padding: '10px 14px', borderRadius: '8px', fontSize: '13px' }}>⚠️ {error}</div>
      )}
        ...
        <Card>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div>
              ...
              <textarea placeholder="e.g. Agent must introduce themselves by name within the first 15 seconds" value={newRule.rule_text} onChange={e => setNewRule(p => ({ ...p, rule_text: e.target.value }))} rows={3} style={{ ...inputStyle, resize: 'vertical' }} />
            </div>
      ...
      {!hasCustomRules && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: '12px', padding: '16px 18px', display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
            ...
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: '13px', fontWeight: '700', color: '#92400E', margin: '0 0 4px' }}>You're currently using the 6 default rules</p>
              <p style={{ fontSize: '13px', color: '#78716C', margin: '0 0 12px', lineHeight: 1.6 }}>These are applied to every call. Customize them or start fresh.</p>
      ...
      {hasCustomRules && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {rules.map((rule, i) => {
            const violationCount = getViolationCount(rule.id)
            return (
              <Card key={rule.id}>
                {edit?.id === rule.id ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  ...
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                      <div style={{ width: '26px', height: '26px', borderRadius: '7px', flexShrink: 0, background: rule.is_active ? '#FFF1F2' : '#F3F4F6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: '800', color: rule.is_active ? '#E11D48' : '#9CA3AF' }}>{i + 1}</div>
                      <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                        <span style={{ fontSize: '11px', fontWeight: '700', padding: '3px 8px', borderRadius: '6px', background: getCategoryInfo(rule.category).color, color: getCategoryInfo(rule.category).textColor, whiteSpace: 'nowrap' }}>
                        ...
                        {violationCount > 0 && (
                          <span style={{ fontSize: '10px', fontWeight: '700', padding: '2px 7px', borderRadius: '99px', background: violationCount >= 3 ? '#FEE2E2' : '#FEF9C3', color: violationCount >= 3 ? '#991B1B' : '#854D0E' }}>
                            ⚠️ {violationCount} violation{violationCount !== 1 ? 's' : ''}
                          </span>
                        )}
                        {violationCount === 0 && rule.is_active && (
                          <span style={{ fontSize: '10px', fontWeight: '700', padding: '2px 7px', borderRadius: '99px', background: '#DCFCE7', color: '#166534' }}>✓ No violations</span>
                        )}
                      </div>
                      <div style={{ display: 'flex', gap: '5px', flexShrink: 0 }}>
                        <button onClick={() => handleToggle(rule.id)} style={{ width: '28px', height: '28px', borderRadius: '7px', background: rule.is_active ? '#F0FDF4' : '#F3F4F6', border: `1px solid ${rule.is_active ? '#BBF7D0' : '#E5E7EB'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '12px' }} title={rule.is_active ? 'Disable' : 'Enable'}>
                          {rule.is_active ? '✓' : '○'}
                        </button>
                        <button onClick={() => setEdit({ id: rule.id, rule_text: rule.rule_text, category: rule.category })} style={{ width: '28px', height: '28px', borderRadius: '7px', background: '#F9FAFB', border: '1px solid #E5E7EB', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '12px' }}>✏️</button>
                        <button onClick={() => handleDelete(rule.id)} style={{ width: '28px', height: '28px', borderRadius: '7px', background: '#FEF2F2', border: '1px solid #FECACA', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '12px' }}
                          onMouseEnter={e => (e.currentTarget.style.background = '#FEE2E2')}
                          onMouseLeave={e => (e.currentTarget.style.background = '#FEF2F2')}
                        >🗑️</button>
                      </div>
                    </div>
                    <p style={{ fontSize: '13px', color: rule.is_active ? '#374151' : '#9CA3AF', lineHeight: 1.6, margin: 0, paddingLeft: '36px', textDecoration: rule.is_active ? 'none' : 'line-through' }}>{rule.rule_text}</p>
                  </div>
                )}
              </Card>
            )
      ...
      {rules.length >= MAX_RULES && (
        <div style={{ background: '#FEF9C3', border: '1px solid #FDE68A', borderRadius: '10px', padding: '12px 16px' }}>
          <p style={{ fontSize: '13px', color: '#92400E', margin: 0 }}>⚠️ Maximum {MAX_RULES} rules reached. Delete a rule to add a new one.</p>
        </div>
      )}
    </div>
  )
...
const inputStyle: React.CSSProperties = { width: '100%', padding: '10px 13px', borderRadius: '10px', border: '1px solid #E5E7EB', fontSize: '13px', outline: 'none', background: '#fff', color: '#111827', fontFamily: "'Plus Jakarta Sans', sans-serif" }